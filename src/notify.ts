...
let itemConfigList: Record<string, unknown>[] = [];
...
fetchClientConfig('/juhuasuan/config/item_config.json').then((data) => {
  itemConfigList = data['config|道具配置'];
});

function enumAsset(type: string) {
  const r = itemConfigList.find((e) => e['item_key|道具key，全局唯一'] === type);
